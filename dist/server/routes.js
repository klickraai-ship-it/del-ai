...
import { subscribers, emailTemplates, campaigns, campaignSubscribers, campaignAnalytics, settings, linkClicks, webVersionViews, users, sessions, paymentProviders, paymentTransactions, termsAndConditions, userTermsAcceptance, insertSubscriberSchema, insertEmailTemplateSchema, insertCampaignSchema, insertPaymentProviderSchema, insertTermsAndConditionsSchema, } from "../shared/schema.js";
...
export async function registerRoutes(app) {
    ...
    app.post("/api/terms/accept", requireAuth, async (req, res) => {
        try {
            ...
            const [existing] = await db
                ...
                .where(and(eq(userTermsAcceptance.userId, userId), eq(userTermsAcceptance.termsId, termsId)))
                .limit(1);
            if (existing) {
    ...
    app.get("/api/subscribers", requireAuth, async (req, res) => {
        try {
            ...
            if (list) {
                filteredResults = results.filter(s => s.lists.includes(list));
            }
    ...
    app.get("/api/subscribers/:id", requireAuth, async (req, res) => {
        try {
            ...
            const [subscriber] = await db
                .select()
                .from(subscribers)
                .where(and(eq(subscribers.id, req.params.id), eq(subscribers.userId, userId)));
            if (!subscriber) {
    ...
    app.patch("/api/templates/:id", requireAuth, async (req, res) => {
        try {
            ...
            const { userId: _, id: __, createdAt: ___, updatedAt: ____, ...allowedUpdates } = req.body;
            // Sanitize HTML content to prevent XSS attacks
            const sanitizedUpdates = { ...allowedUpdates };
    ...
    app.get("/api/campaigns/:id", requireAuth, async (req, res) => {
        try {
            ...
            const [analytics] = await db
                ...
                .where(and(eq(campaignAnalytics.campaignId, req.params.id), eq(campaignAnalytics.userId, userId)));
            ...
            if (campaign.templateId) {
                const [tmpl] = await db
                    .select()
                    .from(emailTemplates)
                    .where(and(eq(emailTemplates.id, campaign.templateId), eq(emailTemplates.userId, userId)));
                template = tmpl;
            }
        ...
        try {
            ...
            if (validatedData.templateId) {
                const [template] = await db
                    .select()
                    .from(emailTemplates)
                    .where(and(eq(emailTemplates.id, validatedData.templateId), eq(emailTemplates.userId, userId)));
                if (!template) {
                    return res.status(403).json({
        ...
        try {
            ...
            if (allowedUpdates.templateId) {
                const [template] = await db
                    .select()
                    .from(emailTemplates)
                    .where(and(eq(emailTemplates.id, allowedUpdates.templateId), eq(emailTemplates.userId, userId)));
                if (!template) {
                    return res.status(403).json({
                        message: "Template not found or access denied. You can only use templates you own."
                    });
    ...
    app.delete("/api/campaigns/:id", requireAuth, async (req, res) => {
        try {
            ...
            await db.delete(campaignSubscribers).where(and(eq(campaignSubscribers.campaignId, req.params.id), eq(campaignSubscribers.userId, userId)));
            await db.delete(campaignAnalytics).where(and(eq(campaignAnalytics.campaignId, req.params.id), eq(campaignAnalytics.userId, userId)));
            const [deleted] = await db
                .delete(campaigns)
                .where(and(eq(campaigns.id, req.params.id), eq(campaigns.userId, userId)))
                .returning();
    ...
    app.get("/api/campaigns/:id/analytics", requireAuth, async (req, res) => {
        try {
            ...
            const clicksData = await db
                ...
                .from(linkClicks)
                .where(and(eq(linkClicks.campaignId, req.params.id), eq(linkClicks.userId, userId)))
            ...
            const webVersionViewsData = await db
                ...
                .where(and(eq(webVersionViews.campaignId, req.params.id), eq(webVersionViews.userId, userId)))
    ...
    app.get('/api/dashboard', requireAuth, async (req, res) => {
        try {
            ...
            const recentCampaigns = await db
                .select()
                .from(campaigns)
                .where(and(eq(campaigns.status, 'sent'), eq(campaigns.userId, userId)))
            ...
            const dashboardData = {
                kpis: [
                    { title: 'Delivery Rate', value: `${deliveryRate}%`, change: '+0.1%', changeType: 'increase', period: 'vs last 7d' },
                    { title: 'Hard Bounce Rate', value: `${bounceRate}%`, change: '-0.05%', changeType: 'decrease', period: 'vs last 7d' },
                    { title: 'Complaint Rate', value: `${complaintRate}%`, change: '+0.02%', changeType: 'increase', period: 'vs last 7d' },
                    { title: 'Unsubscribe Rate', value: `${unsubscribeRate}%`, change: '0.00%', changeType: 'neutral', period: 'vs last 7d' },
                ],
    ...
    app.post("/api/campaigns/:id/send", requireAuth, async (req, res) => {
        try {
            ...
            let targetSubscribers = await db
                .select()
                .from(subscribers)
                .where(and(eq(subscribers.status, 'active'), eq(subscribers.confirmed, true), // Only send to confirmed subscribers (double opt-in)
            eq(subscribers.userId, userId)));
            if (campaign.lists.length > 0) {
                targetSubscribers = targetSubscribers.filter(sub => sub.lists.some(list => campaign.lists.includes(list)));
            }
    ...
    app.get("/api/campaigns/:id/analytics/clicks", requireAuth, async (req, res) => {
        try {
            ...
            const clicks = await db
                .select()
                .from(linkClicks)
                .where(and(eq(linkClicks.campaignId, campaignId), eq(linkClicks.userId, userId)))
    ...
    app.post("/api/settings/email-provider", requireAuth, async (req, res) => {
        try {
            ...
            if (existing) {
                ...
                const newConfig = validatedData.config || {};
                ...
                const encryptedConfig = encryptObject(validatedData.config);
    ...
    app.post("/api/public/subscribe", subscribeRateLimiter, async (req, res) => {
        try {
            ...
            const [existing] = await db
                .select()
                .from(subscribers)
                .where(and(eq(subscribers.email, email.toLowerCase()), eq(subscribers.userId, defaultUserId)));
            if (existing) {
    ...
    app.get("/api/public/unsubscribe/:token", unsubscribeRateLimiter, async (req, res) => {
        ...
        `);
            ...
            const [updated] = await db
                ...
                .where(and(eq(subscribers.id, subscriberId), eq(subscribers.userId, userId)))
                .returning();
            if (!updated) {
                return res.status(404).send(`
          <html>
