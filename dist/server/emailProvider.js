import { sendEmailViaResend } from './resendEmailSender.js';
import { SESEmailService } from './sesService.js';
...
export class UnifiedEmailService {
    async sendEmail(params) {
        ...
        if (!integration || !integration.isActive) {
            ...
            throw new Error(`EMAIL PROVIDER NOT CONFIGURED: User ${userId} must configure an email provider integration before sending emails. ` +
        ...
        switch (integration.provider) {
            ...
            case 'resend':
                ...
                throw new Error(`UNSUPPORTED PROVIDER: Resend does not support per-user credentials yet. ` +
    ...
    async sendViaSES(params, config) {
        ...
        const sesService = new SESEmailService(config.awsAccessKeyId, config.awsSecretAccessKey, config.awsRegion);
        const sesParams = {
            to: params.to,
            from: params.from,
    ...
    async sendViaResend(params, config) {
