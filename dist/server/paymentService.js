...
import { paymentProviders, paymentTransactions } from "../shared/schema.js";
...
class RazorpayProvider {
    ...
    async parseWebhookEvent(headers, body) {
        ...
        const expectedSignature = crypto
            ...
            .digest('hex');
        const isValid = crypto.timingSafeEqual(Buffer.from(webhookSignature), Buffer.from(expectedSignature));
        if (!isValid) {
            return null;
        }
        // Parse event
...
class PayPalProvider {
    ...
    async parseWebhookEvent(headers, body) {
        ...
        if (event === 'PAYMENT.CAPTURE.COMPLETED')
            status = 'captured';
        else if (event === 'PAYMENT.AUTHORIZATION.CREATED')
            status = 'authorized';
        else if (event === 'PAYMENT.CAPTURE.DENIED' || event === 'PAYMENT.CAPTURE.DECLINED')
            status = 'failed';
        return {
            eventType: event,
...
export class PaymentService {
    static async getProvider(providerName) {
        const [provider] = await db
            ...
            .where(and(eq(paymentProviders.provider, providerName), eq(paymentProviders.isActive, true)))
            .limit(1);
        if (!provider) {
            return null;
        }
    ...
    static async createOrder(providerName, amount, currency, email) {
    ...
    static async verifyPayment(providerName, payload) {
        const provider = await this.getProvider(providerName);
        if (!provider) {
            return false;
        }
    ...
    static async processWebhook(providerName, headers, body) {
        const provider = await this.getProvider(providerName);
        if (!provider) {
            return null;
        }
        ...
        const [existingTx] = await db
            ...
            .where(and(eq(paymentTransactions.transactionId, event.transactionId), eq(paymentTransactions.provider, providerName)))
            .limit(1);
        if (existingTx) {
            // Update existing transaction
            const existingData = existingTx.paymentData || {};
            await db
    ...
    static async capturePayment(providerName, orderId) {
        const provider = await this.getProvider(providerName);
        ...
        const baseUrl = paypalProvider.baseUrl;
