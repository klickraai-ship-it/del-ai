import { generateTrackingToken, generateClickTrackingToken, generateUnsubscribeToken, generateWebVersionToken, } from './trackingTokens.js';
/**
 ...
 */
export function replaceMergeTags(content, subscriber, campaign) {
    let result = content;
    ...
    result = result.replace(/\{\{campaign_name\}\}/g, campaign.name ?? '');
    return result;
}
/**
 ...
 */
export function injectTrackingPixel(html, token, domain) {
 ...
 */
export function wrapLinksWithTracking(html, options) {
    const wrappedLinks = [];
 ...
 */
export function injectUnsubscribeLink(html, token, domain) {
    // Remove trailing slash from domain if present
    const cleanDomain = domain.replace(/\/$/, '');
 ...
 */
export function injectWebVersionLink(html, token, domain) {
    // Remove trailing slash from domain if present
    const cleanDomain = domain.replace(/\/$/, '');
 ...
 */
export function processEmailForTracking(content, subscriber, campaign, options) {
    ...
    processedHtml = injectUnsubscribeLink(processedHtml, unsubscribeToken, options.trackingDomain);
    // Step 4: Inject web version link (before link wrapping to avoid wrapping it)
    processedHtml = injectWebVersionLink(processedHtml, webVersionToken, options.trackingDomain);
    ...
    processedHtml = injectTrackingPixel(processedHtml, trackingPixelToken, options.trackingDomain);
